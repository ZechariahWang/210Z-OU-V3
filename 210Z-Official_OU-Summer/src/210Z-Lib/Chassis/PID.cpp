...
double get_angular_error_yes(double target_x, double target_y){
  double x = target_x - utility::get_x();
  double y = target_y - utility::get_y();
...
void TranslationPID::set_translation_pid(double target, 
                                         double maxSpeed, 
                                         bool slew_enabled){
  ...
  while (true){
    ...
    if (slew_enabled){
      slew.initialize_slew(true, maxSpeed, target, ((dt_front_left.get_position() + dt_front_right.get_position()) / 2), ((init_left_pos + init_right_pos) / 2), is_backwards, get_ticks_per_inch());
...
void RotationPID::set_rotation_pid(double t_theta,
                                   double maxSpeed){
...
void CurvePID::set_curve_pid(double t_theta,
                             double maxSpeed, 
                             double curveDamper,
                             bool backwards){
...
void ArcPID::set_arc_pid(double t_x, 
                         double t_y, 
                         double maxSpeed, 
                         double arcDamper){
